...
@foreach (['success', 'info', 'danger'] as $type)
    ...
    @endif
@endforeach
<x-header class="px-6 sm:px-8" title="{{ ucfirst($role->name) . ' Permissions' }}"
     sub-title="Manage all permissions or add a new one" />
     ...
     <div>
          ...
          <x-form :space="false">
               ...
               <x-form.select name="perPage" onchange="submit()">
                    <option {{ $rolesPermissions->perPage() == 10 ? 'selected' : '' }} value="10">10</option>
                    <option {{ $rolesPermissions->perPage() == 25 ? 'selected' : '' }} value="25">25</option>
                    <option {{ $rolesPermissions->perPage() == 50 ? 'selected' : '' }} value="50">50</option>
                    <option {{ $rolesPermissions->perPage() == 100 ? 'selected' : '' }} value="100">100</option>
               </x-form.select>
          </x-form>
     </div>
